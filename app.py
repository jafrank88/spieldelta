...
CSV_COLUMN = os.getenv("TABLETOP_CSV_COLUMN", "")
...
def fetch(url, accept):
    try:
        response = requests.get(url, headers={"Accept": accept, "User-Agent": USER_AGENT}, timeout=(10, 60))
...
def get_csv_titles():
    ...
    try:
        ...
        if column is None:
            column = next((h for h in headers if any(w in (h or "").casefold() for w in ("title", "name", "game"))), headers[0])
...
def cached_data(name, loader):
    ...
    data, error = loader()
    with _cache_lock:
        _cache[name] = (time.monotonic() + (ERROR_TTL if error else CACHE_TTL), data, error)
    return data, error
...
def closest_match(key, by_len, threshold):
    factor = threshold / (200 - threshold)
...
def missing_from_csv(novelties, csv_titles, threshold=FUZZY_THRESHOLD):
    ...
    for item in novelties:
        ...
        missing.append({
            ...
            "closest_score": score,
            "note": note,
            "bgg_url": "https://boardgamegeek.com/geeksearch.php?action=search&objecttype=boardgame&q=" + quote_plus(item["title"]),
        })
...
th,td{border:1px solid #ccc;padding:8px;text-align:left;vertical-align:top} th{background:#f2f2f2}
.warning{color:#8a3b00;background:#fff3e0;padding:10px;border:1px solid #ffcc80;margin-bottom:20px}
.muted{color:#666;font-size:.9em} button{cursor:pointer} #filter{padding:6px;width:min(100%,320px);margin:8px 0}
.sort-button{padding:6px;margin:8px 4px 8px 0}.sort-button.active{font-weight:bold;background:#dbeafe}
</style></head><body>
...
{% if missing %}
<div aria-label="Sort results">
  <span>Sort alphabetically by:</span>
  <button type="button" class="sort-button active" data-sort="name" onclick="sortRows('name', this)">Name</button>
  <button type="button" class="sort-button" data-sort="exhibitor" onclick="sortRows('exhibitor', this)">Exhibitor</button>
</div>
<input id="filter" type="search" placeholder="Filter this list…" oninput="filterRows(this.value)">
<table id="results"><thead><tr><th>SPIEL title</th><th>Exhibitor</th><th>BoardGameGeek</th><th>SPIEL page</th></tr></thead><tbody>
{% for item in missing %}<tr>
...
{% elif not spiel_error and not csv_error %}<p>Every SPIEL novelty has a match in your CSV.</p>{% endif %}
<script>
...
function copyAndOpen(btn) {
  ...
  if (navigator.clipboard && navigator.clipboard.writeText) navigator.clipboard.writeText(title).then(done, () => prompt("Copy this title:", title));
  else prompt("Copy this title:", title);
}
function sortRows(field, clickedButton) {
  ...
  document.querySelectorAll(".sort-button").forEach(button => button.classList.toggle("active", button === clickedButton));
}
function filterRows(q) {
  q = q.toLowerCase();
  document.querySelectorAll("#results tbody tr").forEach(row => { row.style.display = row.textContent.toLowerCase().includes(q) ? "" : "none"; });
}
</script></body></html>
"""
...
def index():
    ...
    return render_template_string(TEMPLATE, missing=missing, spiel_count=len(spiel), csv_count=len(csv_titles), spiel_error=spiel_error, csv_error=csv_error, novelties_url=NOVELTIES_URL)
